...
public class Homepage extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(request.getParameter("visitedUser")!=null){
            ...
            user= request.getParameter("visitedUser");
            utente = UtenteFactory.getInstance().getUtenteByUsername(user);
            session.setAttribute("visitedUser", user);
        }else{
            
            user = session.getAttribute("visitedUser").toString();
            utente = UtenteFactory.getInstance().getUtenteByUsername(user);
        }
        ...
        if(utente !=null){
            ...
            if(Validate.areFriends(utente.getUsername(), visitor.getUsername())
                    || Validate.areFriends(visitor.getUsername(), utente.getUsername() )){
            trueFriends = true;
